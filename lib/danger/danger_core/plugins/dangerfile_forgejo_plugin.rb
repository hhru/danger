...
module Danger
  ...
  # @tags core, github
  #
  class DangerfileForgejoPlugin < Plugin
    # So that this init can fail.
    def self.new(dangerfile)
      return nil if dangerfile.env.request_source.class != Danger::RequestSources::Forgejo
    ...
    def initialize(dangerfile)
      super(dangerfile)
      @forgejo = dangerfile.env.request_source
    end
    ...
    #
    def pr_labels
      @forgejo.issue_json["labels"].map { |l| l[:name] }
    end
    ...
    def dismiss_out_of_range_messages(dismiss = true)
      if dismiss.kind_of?(Hash)
        @forgejo.dismiss_out_of_range_messages = dismiss
      elsif dismiss.kind_of?(TrueClass)
        @forgejo.dismiss_out_of_range_messages = true
