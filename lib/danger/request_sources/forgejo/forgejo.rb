...
require "danger/request_sources/forgejo/forgejo_review_unsupported"
  ...
  module RequestSources
    class Forgejo < RequestSource
      ...
      def pr_diff
        ...
        # as the Forgejo API for listing pull request files is missing file names in the patch.
        ...
        files = client.pull_request_files(
          ...
          accept: "application/vnd.forgejo.v3.diff"
        )
      ...
      def review
        ...
        begin
          @review = client.pull_request_reviews(ci_source.repo_slug, ci_source.pull_request_id)
                          .map { |review_json| Danger::RequestSources::ForgejoSource::Review.new(client, ci_source, review_json) }
                          .select(&:generated_by_danger?)
                          .last
          @review ||= Danger::RequestSources::ForgejoSource::Review.new(client, ci_source)
          @review
        rescue Octokit::NotFound
          @review = Danger::RequestSources::ForgejoSource::ReviewUnsupported.new
          @review
        end
      end

      def setup_danger_branches
        # we can use a forgejo specific feature here:
      ...
      def issue_comments
        @comments ||= client.issue_comments(ci_source.repo_slug, ci_source.pull_request_id)
                            .map { |comment| Comment.from_forgejo(comment) }
      end

      # Sending data to Forgejo
      ...
      def submit_pull_request_status!(warnings: [], errors: [], details_url: [], danger_id: "danger")
        ...
        begin
          ...
          # use a read-only Forgejo account
          if errors.count > 0
      ...
      def delete_old_inline_violations(danger_comments: [], non_danger_comments: [])
        danger_comments.each do |comment|
          ...
          if !violation.nil? && violation.sticky
            body = generate_inline_comment_body("white_check_mark", violation, danger_id: danger_id, resolved: true, template: "forgejo")
      ...
      def submit_inline_comments_for_kind!(kind, messages, diff_lines, danger_comments, previous_violations, danger_id: "danger")
        ...
        messages.reject do |m|
          ...
          if is_markdown_content
            body = generate_inline_markdown_body(m, danger_id: danger_id, template: "forgejo")
          else
            # Hide the inline link behind a span
            m = process_markdown(m, true)
            body = generate_inline_comment_body(emoji, m, danger_id: danger_id, template: "forgejo")
          ...
          if matching_comments.empty?
            begin
              ...
              # See https://forgejo.com/danger/danger/issues/1475 for detailed information.
      ...
      def parse_message_from_row(row)
        message_regexp = %r{(<(a |span data-)href="https://#{host}/#{ci_source.repo_slug}/src/commit/[0-9a-z]+/(?<file>[^#]+)#L(?<line>[0-9]+)"(>[^<]*</a> - |/>))?(?<message>.*?)}im
      ...
      def markdown_link_to_message(message, hide_link)
        url = "https://#{host}/#{ci_source.repo_slug}/src/commit/#{pr_json['head']['sha']}/#{message.file}#L#{message.line}"
      ...
      def file_url(organisation: nil, repository: nil, ref: nil, branch: nil, path: nil)
        ...
        begin
          ...
          # Fallback to forgejo.com
          ref ||= "master"
          @download_url = "https://raw.forgejousercontent.com/#{organisation}/#{repository}/#{ref}/#{path}"
        end
      end
